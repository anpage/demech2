...
function(reccmp_build_yml RESULT PROJECT_DIR BASE)
    set(txt "project: '${PROJECT_DIR}'\ntargets:\n")
...
function(reccmp_configure)
    ...
    reccmp_build_yml(build_yml_txt "${reccmp_project_dir}" "${outputdir}")
    file(GENERATE OUTPUT "${outputdir}/reccmp-build.yml" CONTENT "${build_yml_txt}")

    if(ARGS_COPY_TO_SOURCE_FOLDER)
        reccmp_build_yml(source_yml_txt "${reccmp_project_dir}" "${CMAKE_SOURCE_DIR}")
        file(GENERATE OUTPUT "${CMAKE_SOURCE_DIR}/reccmp-build.yml" CONTENT "${source_yml_txt}" CONDITION $<CONFIG:Release>)
    endif()
endfunction()
